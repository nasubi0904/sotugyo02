...
### 6.1 プロジェクト配下の相対パス定義ルール
プロジェクト配下の相対パスは、環境定義や設定ファイルで人が直接記述しても安全に解決できるよう、以下の共通仕様に統一する。

**✅ 記述フォーマット（推奨）**
- `project://<relative-path>` 形式で表記する。
- `<relative-path>` は **プロジェクトルートを基準** にした相対パスとし、区切りは **常に `/`** を用いる。
- 先頭 `/` は付与しない（例: `project://assets/models/hero.fbx`）。

**✅ 保存時の正規化手順**
1. 入力が `Path` の場合は `project_root` から `relative_to()` で相対化する。
2. 相対化に成功したものだけを `project://` 形式へ変換する。
3. `Path.as_posix()` を使い **OS 依存の区切り** を排除して保存する。

**✅ 読み込み時の復元手順**
1. `project://` で始まる文字列を検出し、接頭辞を取り除いた相対パスを取得する。
2. `Path(project_root) / relative_path` で絶対パスを再構築する。
3. `resolve()` で正規化した後、`project_root` 配下であることを `relative_to()` で確認する。
4. 配下判定に失敗した場合は **無効パスとして扱い警告** する。

**✅ 互換運用（既存データ向け）**
- `project://` 接頭辞が無くても **相対パス** であれば従来通り許可する。
- ただし新規保存時は必ず `project://` 形式に変換し、データを正規化する。

**✅ 実装時の責務分離**
- 相対パスの変換・検証は `infrastructure.paths` に専用ヘルパー（例: `resolve_project_path` / `to_project_relative_spec`）として集約する。
- UI / ドメイン / 解析処理はヘルパーを通じて共通ロジックを利用し、独自実装を避ける。
