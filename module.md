...
## ファイルパス取り扱い調査（2026-01-23）
プロジェクト配下の相対パスを安全に再構築するため、ファイルパスを扱う主要処理を棚卸しした。

- **プロジェクト設定・レジストリ**: `ProjectSettings` と `ProjectRegistry` がプロジェクトルートを永続化し、UI からの読み込み時に `Path` へ復元する。
- **プロジェクト構成検証**: `domain/projects/structure` が `Path.relative_to()` を使って欠落ディレクトリ・ファイルを報告する。
- **UI のファイルノード**: `node_editor.py` がプロジェクト配下のコピーと相対パス化、`_is_project_relative_file_path` による安全判定を担当する。
- **コンテンツブラウザ**: `content_browser.py` がファイル/フォルダをノード化する際に絶対パスを保持する。
- **ツール環境・Rez**: `domain/tooling` と `scripts/rez_launch.py` が実行ファイル・パッケージルートの解決を行い、環境変数や `Path` へ変換する。
- **設定保存先の解決**: `infrastructure/paths/storage.py` が OS 別に設定ディレクトリを解決する。

相対パスの保存・復元ロジックが UI とドメインに分散しているため、**共通ヘルパーに集約した上で `project://` 形式を正として扱う**方針を立てた（詳細は `plan.md` に記載）。
...
最終更新日: 2026-01-23
- 2026-01-23: ファイルパス処理の棚卸しと `project://` 形式によるプロジェクト相対パス仕様の採用方針を整理した。
