...
最終更新日: 2026-04-05
- 2026-04-05: NTFS ジャンクション作成は Windows の `mklink /J` を用いる必要があり、`cmd /c mklink /J <link> <target>` で生成する方針を整理した。
- 2026-04-05: ユーザーローカル配下の `nodes` にノードごとのコピー、同階層の `junc` にノードグラフ構造を再現するディレクトリを生成する方針を追記した。
