...
- 2026-03-30: Rez 処理は Python API に統一し、実行ファイルや埋め込み環境のパスに依存しない運用方針を追記した。

最終更新日: 2026-03-31
- 2026-03-31: ツール環境定義の JSON からノード入力トークン（`{{input:...}}`）を抽出して入力ポートを追加する方針と、KDMenvs 配下の環境定義をノードカタログへ反映する流れを整理した。
- 2026-03-31: ツールノード起動時に `{{input:...}}` トークンを解決し、環境変数と起動引数を Rez 起動へ渡す設計手順を追記した。
- 2026-03-31: 起動通知に起動コマンドと環境変数を必ず表示する方針を追記した。
- 2026-03-31: 起動引数は EXECUTE_ で解決した起動コマンドへ追記する方針に統一した。
- 2026-03-31: 起動引数のトークン解決時に空白を含むパスを自動で引用符で囲む方針を追記した。

最終更新日: 2026-04-01
- 2026-04-01: コンテンツブラウザの環境定義フォルダ配下に「作成した環境」「登録ツール」を既定生成し、`tool-environment:rez:` を登録ツールへ分類する運用方針を確認した。
...
最終更新日: 2026-04-05
- 2026-04-05: ノードローカルコピーと NTFS ジャンクション生成を追加するため、ユーザーローカルの `nodes`/`junc` 管理と入力側ノード構造の再現方針を整理した。

最終更新日: 2026-04-06
- 2026-04-06: ジャンクション生成の入力階層を同階層へ統一し、同名衝突時はユーザー選択で既存利用/別名作成/キャンセルを行う方針を整理した。

最終更新日: 2026-04-07
- 2026-04-07: ジャンクションをノード階層のディレクトリとして生成し、入力側ノードを配下へネストする構造へ変更した。

最終更新日: 2026-04-08
- 2026-04-08: FileNode のポート設定を再確認し、入力の `multi_input` と出力の `multi_output` を明示して複数接続を許可する方針を整理した。

最終更新日: 2026-04-09
- 2026-04-09: 「ローカルで開く」実行時にローカルコピー完了後にジャンクションを生成する2段階フローへ整理し、安全性を高める方針を確認した。


最終更新日: 2026-02-12
- 2026-02-12: ファイルノードのパス操作（参照・ローカルで開く・パス選択/検証）をツール環境ノードへ安全に統合するため、共通のパス制御ヘルパーを設けて `tool_output_dir` を同一導線で扱う方針を整理した。
- 2026-02-12: ツール環境ノードの基本ポートを複数接続対応（`multi_input` / `multi_output`）へ揃え、ファイルノードと同等の接続性を持たせる方針を追記した。

最終更新日: 2026-04-11
- 2026-04-11: ジャンクション生成 `_build_junction_tree` の巡回制御を確認し、ノード単位 `visited` では合流 DAG の共有ノードが片側欠落するため、再帰経路ベースの循環検出へ変更する方針を整理した。
- 2026-04-11: 分岐→合流（共有上流ノードの重複展開）と循環（無限再帰防止）の2ケースを回帰テスト化し、安全に構造展開できることを確認する方針を追記した。
