...
class DateNodeItem(BackdropNodeItem):
    ...
    def __init__(self, name: str = "backdrop", text: str = "", parent=None):
        self._snap_grid_size: float = self.DEFAULT_SNAP_GRID
    ...
    def on_sizer_pos_changed(self, pos: QtCore.QPointF) -> None:
        ...
        snapped_pos = QtCore.QPointF(
            width - self._sizer.size, height - self._sizer.size
        )
    ...
    def _snap_value(self, value: float) -> float:
        if self._snap_grid_size <= 0:
            return value
        return round(value / self._snap_grid_size) * self._snap_grid_size
