...
### Nuke テンプレート検出で実行ファイル候補が拾えない
- **現象**: Nuke のインストールディレクトリ内に `Nuke13.2.exe` などの実行ファイルが存在しても、テンプレート探索が候補を検出できなかった。
- **原因**: 実行ファイル探索が `フォルダ名 + ".exe"` と `Nuke.exe` のみを前提としており、`Nuke*.exe` の命名差分を考慮していなかった。
- **対応**: ディレクトリ内で `Nuke*.exe` をグロブ検索し、バージョン降順で最初の一致を採用するよう更新した。従来の `entry.name + ".exe"` / `Nuke.exe` のフォールバックは維持した。
