...
### ツールノードの起動ボタン不足
- **現象**: ツール環境ノードを選択してもインスペクタ上に起動導線がなく、rez_info を使ったツール起動が行えなかった。
- **原因**: ノードインスペクタがプロパティ表示のみで、Rez パッケージ起動のアクションを配置していなかった。
- **対応**: プロパティタブに起動ボタンを追加し、rez_info からパッケージ名とバージョンを読み取って `scripts/rez_launch.py` を呼び出す導線を追加した。
