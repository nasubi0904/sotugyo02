...
### 要求プラグインのツール相対化ボタンと絶対パス化
- **現象**: 要求プラグインの相対化が自動判定のみで、ツール実体パス基準の相対化や絶対パスへ戻す操作が UI からできなかった。
- **原因**: ツール実体パス配下の相対化ロジックが存在せず、プラグイン一覧に相対/絶対の切り替え操作も未提供だった。
- **対応**: ツール実体パスのディレクトリ配下であれば自動的にツール相対 (`tool:relative`) として保持し、選択中プラグインを「ツールから相対化」「絶対パス化」で切り替えられるようにした。ツール実体パスが解決できない場合や配下外は警告を表示する。
