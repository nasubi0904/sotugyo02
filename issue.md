...
### ノード幅の自動伸長と色の区別不足（2026-02-07）
- **現象**: メモノード以外のノードが名前の長さに応じて横幅が伸び、縦グリッド幅と一致しないため整列感が崩れていた。ノード種別ごとの色分けが不足し、判別しづらい状態だった。
- **原因**: デモノードやファイル/ツール環境ノードに固定幅設定がなく、NodeGraphQt の既定幅がノード名に追従していた。また、デモノードに色設定が存在せず他ノードと色が近接していた。
- **対応**: ノード幅を共通の固定値へ統一し、背景縞幅（縦グリッド）との整合を取った。併せてノード種別ごとに色を明示的に設定し、視認性を改善した。
