...
### Rez コマンド PATH が未設定の環境で起動できない
- **現象**: Rez パッケージノードを選択し「Rez 環境で起動」を押下すると、パッケージは存在するにもかかわらず「rez コマンドが見つかりません」と警告され起動できない。
- **原因**: RezEnvironmentResolver は `SOTUGYO_REZ_PATH` 環境変数しか PATH 追加入り口を持たず、パッケージの保存場所から Rez のヒントパスを組み立てていなかったため、環境変数未設定のマシンでは Rez CLI を検出できなかった。
- **対応**: ノードの `package_path` をヒントに KDMrez 配下を `SOTUGYO_REZ_PATH` へ自動付与する `inject_package_path_hint` を追加し、UI から解決前に適用することで Rez が PATH にない環境でも検出を試みるようにした。
