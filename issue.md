...
### Rez 環境起動でシステム側 rez-env が混在する
- **現象**: ツール起動時に現在の Python 環境ではなく、PATH 上にある別の rez-env が利用される可能性があった。
- **原因**: `rez_launch.py` の rez-env 解決が PATH を優先する順序になっており、システム側 rez-env を拾うケースがあった。
- **対応**: rez-env の探索を現在の Python 実行環境に限定し、hint 指定も環境内の実体に一致する場合のみ許可するよう修正した。
