...
### ローカルで開く時のジャンクション生成が段階的でない（2026-04-09）
- **現象**: 「ローカルで開く」実行時にノードごとにコピーとジャンクション生成が混在し、途中で失敗した場合に中途半端なジャンクション構造が残る恐れがあった。
- **原因**: ジャンクション構築処理がコピーと生成を同一トラバーサル内で順次実行しており、安全な二段階処理が保証されていなかった。
- **対応**: 先に全ノードのローカルコピーを確実に準備し、完了後にジャンクションを作成する二段階フローへ変更した。コピー未完了の場合は警告して中断することで整合性を保つよう整理した。
