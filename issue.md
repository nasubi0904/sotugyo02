...
### 自動終了の成功扱いによる起動テスト誤判定
- **現象**: 自動終了フラグ付きの起動テストが成功扱いとなり、手動終了のみ成功とする運用方針と食い違っていた。
- **原因**: `_run_application` が `auto_exit` を成功扱いのまま終了コード 0 で返していた。
- **対応**: `auto_exit` の場合は終了コードを 1 として扱い、手動終了以外を成功にしない判定へ変更した。
