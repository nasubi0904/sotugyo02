...
### テンプレート探索で生成する Rez パッケージに実行ファイル名が記録されない
- **現象**: テンプレート探索で自動生成した Rez パッケージに、起動対象の .exe ファイル名が記録されず運用メモとして残せなかった。
- **原因**: `package.py` の生成ロジックが name/version のみを書き出しており、実行ファイル名をコメントとして付記する経路が存在しなかった。
- **対応**: テンプレート探索で生成する場合のみ `#EXECUTE=<exe>` コメントを `package.py` に付与するオプションを追加した。
