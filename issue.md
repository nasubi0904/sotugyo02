...
### Rez 環境起動時のバージョン不一致による起動失敗
- **現象**: ツール環境ノードの `rez_info` に `version=2024` を指定しても、ローカルには `2024.0` などのバージョンしか存在せず、`rez-env` 実行が失敗して起動エラーとなる。
- **原因**: 起動前の検証が「パッケージ名の存在」に留まり、指定バージョンの実在チェックや代替バージョンの選択を行っていなかった。
- **対応**: `RezPackageRepository.list_package_entries()` を用いてバージョン一覧を取得し、指定バージョンの一致・プレフィックス一致・最新バージョンの順で解決するようにした。解決結果は UI へ反映し、必要に応じて起動時の注意文言を表示する。
