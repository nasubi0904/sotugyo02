...
### 起動環境構成で要求プラグインの管理が不足
- **現象**: 起動環境構成ダイアログで要求プラグインを管理する UI がなく、環境変数の設定前にプラグイン一覧を保持できなかった。
- **原因**: 起動環境の編集 UI が環境変数・起動引数のみを扱う設計のままで、プラグイン名とファイルパスの保持や Windows Known Folder API での相対化を考慮していなかった。
- **対応**: 環境変数の設定前に要求プラグインの列挙エリアと追加ボタンを追加し、複数選択時の拡張子統一チェックと Known Folder API を使ったパス属性（absolute/known）付き保存を実装した。
