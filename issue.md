...
### 起動環境編集で要求プラグインと Known Folder の指定ができない
- **現象**: 起動環境編集ダイアログに要求プラグインの一覧と追加導線がなく、環境変数の前にプラグイン設定を挿入できなかった。また、プラグインパスを別 PC でも再構築できる形式で保持する手段がなかった。
- **原因**: UI が環境変数・起動引数の入力のみを前提としており、複数プラグイン選択時の拡張子統一や Known Folder API による相対化ロジックが未実装だった。
- **対応**: 環境変数の前に要求プラグイン設定エリアを追加し、追加ボタンから複数選択を許可する導線を実装した。Known Folder のチェックにより相対化可能なパスは `known` 属性と GUID 識別子＋相対パスで保持し、相対化できない場合やチェックオフ時は `absolute` を保持するようにした。
