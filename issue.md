...
### コンテンツブラウザのドラッグ配置とインスペクタ表示の不整合
- **現象**: コンテンツブラウザからドラッグ&ドロップでノードを配置できず、ダブルクリック時に同じノードが二重に生成されることがあった。さらにインスペクタのプロパティ欄でツール起動ボタンが全ノードに表示され、関係ない UI が露出していた。
- **原因**: アイコン一覧側でドラッグ用 MIME を送出しておらず、ノードエディタ側でもドロップイベントを受け取る導線が無かった。ダブルクリックは短時間に同一イベントが複数回伝播するケースがあり、連続発火を抑止していなかった。インスペクタはツール環境ノード以外の表示制御を行っていなかった。
- **対応**: コンテンツブラウザ側でノード種別を MIME へ追加してドラッグを開始し、ノードエディタでドロップ位置へ生成するイベントフィルタを追加した。ダブルクリックは直近のノード種別と時刻を保持して連続発火をガードした。インスペクタではツール起動 UI をツール環境ノードのみに表示するよう切り替えた。
