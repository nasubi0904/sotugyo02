...
- **現象**: コンテンツブラウザのノード一覧がテキスト中心のツリー表示で、アイコンを起点にしたファイルシステム型の操作感が弱く、ノード追加が選択直後に発火することで誤操作が起きやすかった。
- **原因**: QTreeView の既定描画に依存し、一覧をファイルタイル風に描画するデリゲートが存在しなかった。さらに選択変更でノード追加を実行していたため、選択操作と確定操作が分離されていなかった。
- **対応**: ノードエントリ描画用のデリゲートを追加し、アイコン＋タイトル＋サブタイトルのタイル表現へ刷新した。ジャンル項目にフォルダアイコンを付与し、ノード追加はアクティベート（ダブルクリック／Enter）時のみ発火するよう変更してファイルブラウザに近い操作感へ整理した。
