...
### ツールノードのローカルコピー/ジャンクション表示不足（2026-04-09）
- **現象**: ツールノードを選択しても、インスペクタの「ローカルへ移動」「ローカルで開く」欄が無効のままになり、ファイルノードと同様のローカルコピーやジャンクション表示を行えなかった。
- **原因**: ローカルコピーとジャンクション表示のUI更新・実行処理がファイルノード専用の判定になっており、`ToolEnvironmentNode` を対象に含めていなかった。
- **対応**: ツールノードもローカルコピー/ジャンクション生成の対象に追加し、ディレクトリを保持するツールノードであれば同じ導線を使えるよう更新した。

### ローカルで開く時のジャンクション生成が逐次処理になる（2026-04-10）
- **現象**: 「ローカルで開く」を実行すると、入力ノードごとにコピーとジャンクション生成が同時進行し、コピー途中の状態でジャンクションが作成される可能性があった。
- **原因**: ジャンクションツリーの構築中にノード単位でコピー確認とジャンクション作成を行う実装になっており、全ノードのコピー完了を待つ設計になっていなかった。
- **対応**: ジャンクション生成の前に対象ノード全体のローカルコピーを先行確保し、コピー完了後にまとめてジャンクションを生成する 2 ステップ方式へ変更した。
