...
### ノード幅がラベルで伸びて見た目が不統一（2026-04-05）
- **現象**: ノード名が長い場合に横幅が自動拡張され、グリッドに沿った見た目が崩れる。
- **原因**: NodeGraphQt の既定挙動でラベル長に合わせてノード幅が変動し、固定幅を明示していなかった。
- **対応**: メモノード以外は `set_property("width", ...)` で横幅をグリッド単位に固定し、ノード種別ごとの配色も強化して判別しやすくした。
