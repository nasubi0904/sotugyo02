...
### ツール環境定義ファイルの読み込み不足
- **現象**: KDMenvs 配下に JSON/YAML/TOML の環境定義を保存しても、起動時や一覧更新時にツール環境ノードへ反映されない。
- **原因**: `ToolEnvironmentService` が環境ディレクトリを走査しておらず、保存済み定義が `ToolConfigRepository` へ統合されていなかった。
- **対応**: KDMenvs を走査して環境定義を読み込み、`ToolEnvironmentDefinition` に変換して `ToolConfigRepository.save_all()` に統合する同期処理を追加した。

### 起動環境編集ダイアログの保存未実装
- **現象**: 起動環境編集ダイアログで入力した内容がファイルへ保存されず、一覧を更新しても新しい環境定義が表示されない。
- **原因**: 保存ボタンが JSON 出力のデバッグ用途に留まっており、KDMenvs への書き込み処理や一覧再読み込みが未実装だった。
- **対応**: ダイアログで環境定義ファイルを保存し、保存後に一覧を再読み込みする処理を追加した。

### 起動時のツール構成同期順序の不足
- **現象**: 起動時のツール構成同期で環境定義ファイルが反映されず、ツール環境ノードが最新状態にならないことがあった。
- **原因**: 起動直後にローカル Rez パッケージの更新より先に環境スナップショットを構築しており、同期順序が逆になっていた。
- **対応**: ツール構成の更新でローカル Rez パッケージの更新を先に実施し、その後に環境スナップショットを構築するように順序を調整した。
