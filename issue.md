...
### 2026-03-31 ツール環境編集でノード入力パスを扱えない
- **現象**: 起動環境の編集で要求ファイル・環境変数・起動引数にファイルノード由来の入力を指定できず、ノード入力として保存してもツール環境ノードへ反映されなかった。
- **原因**: 環境定義ファイルの読み書きとツール環境ノードのカスタムプロパティ同期が未実装で、ノード入力プレースホルダや入力ポート生成に対応していなかった。
- **対応**: 環境定義ファイルにノード入力プレースホルダとトークン情報を保存し、ツール環境ノード生成時にカスタムプロパティへ格納して入力ポートを自動追加するよう統合した。
