...
### プロジェクト相対パスの定義規約不足
- **現象**: プロジェクト配下の相対パスを環境定義などで記述する際、統一された書式がなく、読み込み側の再構築処理が機能追加ごとに分散する懸念があった。
- **原因**: 相対パスの保存・復元が UI／ドメインの個別実装に依存しており、共通仕様がドキュメント化されていなかった。
- **対応**: `project://` 形式の相対パス仕様と安全な復元手順を `plan.md` に明記し、共通ヘルパーへ集約する方針を定義した。
