...
### ツール環境定義ファイルの反映漏れと同期順序
- **現象**: KDMenvs 配下に保存した環境定義（json/yaml/toml）が起動時のツール環境一覧へ反映されず、編集ダイアログで保存しても一覧が更新されなかった。
- **原因**: ツール環境の同期処理が Rez パッケージ由来の定義のみを生成しており、環境定義ファイルを読み込んで `ToolConfigRepository.save_all()` に統合する経路が欠落していた。さらに編集ダイアログがファイル保存を行わず、ノードエディタ側の同期順序も更新前提の情報を先に参照していた。
- **対応**: `get_tool_environment_dir()` から json/yaml/toml を読み込むローダを追加し、環境定義を `ToolEnvironmentDefinition` へ変換して保存対象に統合した。編集ダイアログで環境定義をファイル保存し一覧を再読込するよう改善し、起動時同期の順序を調整して反映漏れを解消した。
