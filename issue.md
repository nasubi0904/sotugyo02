...
### Rez 環境の起動失敗（EXECUTE_ 変数取得）
- **現象**: ツール起動時に「Rez 環境の起動に失敗しました: EXECUTE_ 変数の取得に失敗しました（rez-env returncode=1）」が表示され、起動できない。
- **原因**: Rez 環境内の EXECUTE_ 変数取得を `rez-env <pkg> -- python -c ...` に依存していたため、Rez が Python 実行環境へ干渉するケースで returncode=1 となり、EXECUTE_ 変数の取得に失敗していた。
- **対応**: まず Python での取得を試行し、失敗時は `rez-env <pkg> -- cmd /c set`（非 Windows は `env`）へフォールバックして環境変数から EXECUTE_ を抽出するよう修正した。これにより Python 実行が失敗しても起動先の EXECUTE_ 変数を取得できるようになった。
