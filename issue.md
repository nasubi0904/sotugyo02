...
### 起動環境構成ダイアログの UI 定義不足
- **現象**: 起動環境の構成画面が再構成中のプレースホルダ表示のみで、既存環境一覧や環境変数・起動引数コネクタの入力 UI が存在しなかった。
- **原因**: UI 再設計の前段で旧レイアウトを撤去したまま、環境一覧と編集モーダルの構成を復元できていなかった。
- **対応**: 環境一覧と追加/編集導線を実装し、ツールパッケージ選択と色空間候補のプルダウン、環境変数・起動引数コネクタ入力欄を備えたモーダル UI を追加した。確定ボタンはデモとして確定内容をコンソールへ出力するようにした。
