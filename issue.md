...
### ファイルノードのローカルコピー/ジャンクション導線不足（2026-04-05）
- **現象**: ディレクトリを保持するノードにローカルコピー導線がなく、ファイルノードの入力側構造に沿ったジャンクション生成も行えなかった。
- **原因**: インスペクタに「ローカルへ移動」「ローカルで開く」ボタンが存在せず、ユーザーローカルの `nodes` / `junc` を生成・同期するロジックが未実装だった。
- **対応**: インスペクタに新ボタンを追加し、ローカルコピーの不足時は確認ダイアログでコピーを促すフローを実装した。`nodes/<uuid>` へのコピーと `junc` 配下での入力側構造の再現を行い、Windows ではジャンクション (`mklink /J`) を生成してエクスプローラーで表示する導線を追加した。
