...
### ツールノードのローカル起動ボタンとローカル最適化（2026-04-10）
- **現象**: ツールノードの起動導線が通常起動のみで、入力側パスやツールノードディレクトリをローカルへ最適化した起動が行えなかった。
- **原因**: インスペクタにローカル起動用のボタンが存在せず、起動処理もローカルコピー済みのパスを利用する分岐を持っていなかった。
- **対応**: インスペクタに「ローカルで起動」ボタンを追加し、入力側ファイルノードとツールノードディレクトリをローカルコピーへ解決したうえで起動引数/環境変数を構築する処理を追加した。
