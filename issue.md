...
### ツール環境ファイルの反映不足
- **現象**: KDMenvs に保存された環境定義ファイル（json/yaml/toml）がツール環境一覧に反映されず、起動時のスナップショットにも反映されない。
- **原因**: `ToolEnvironmentService` が環境ディレクトリを読み込む処理を持たず、UI 側でも編集結果をファイルへ保存していなかった。さらに、起動時の同期順序が Rez パッケージ一覧の更新と噛み合っていなかった。
- **対応**: 環境ディレクトリのファイル読み込みを追加し、`ToolEnvironmentDefinition` へ変換して設定保存へ統合した。編集ダイアログでは JSON/YAML/TOML 保存を実装し、保存後に一覧を再読込するよう変更した。起動時同期では Rez パッケージ読み込みを先に実行し、最新の環境定義がスナップショットへ反映されるよう順序を調整した。
