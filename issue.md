...
### 起動環境構成ダイアログの二領域 UI 実装
- **現象**: Rez commands と起動引数コネクタ定義を同一画面で編集できず、環境ノード登録に必要な項目が分断されていた。
- **原因**: 起動環境構成ダイアログが再構成中のプレースホルダ表示に留まり、ヘッダー情報・Rez commands・コネクタ定義の編集 UI が未実装だった。
- **対応**: ヘッダー入力（ノード名/ID/対象パッケージ/説明/スキーマ）と Rez commands 編集領域、起動引数コネクタ定義領域（テーブル/プレビュー/JSON）を 1 ウィンドウ内で明確に分離して提供した。保存処理は未実装とし、検証とプレビューを UI 内に実装した。
