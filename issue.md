...
### ノード幅の自動伸長と色分け不足（2026-04-05）
- **現象**: ノード名が長いとノード幅が伸び、ノード種類ごとの識別もしづらかった。
- **原因**: ノード生成時に共通の幅設定がなく、NodeGraphQt の自動サイズに依存していた。また、一部ノードは既定色のままで種類差が目立たなかった。
- **対応**: 主要ノードに共通幅（256）を設定し、日付ノードはグリッド幅に合わせて丸めるよう調整した。あわせてノード種別ごとに配色を割り当て、識別しやすくした。
