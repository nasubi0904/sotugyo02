...
#### ツール環境の識別子整理と保存形式の簡素化
- **現象**: ツール環境の識別に `environment_id` を使っていたため、Rez パッケージ構成が同一でも別環境として扱われ、更新時に冗長な ID 依存が残っていた。
- **原因**: 保存形式が `environment_id` を前提としており、Rez パッケージ集合で同一性を判定する仕組みが欠けていた。
- **対応**: ツール環境定義を最小構成（name / rez_packages など）へ縮小し、Rez パッケージ集合をキーとする更新・削除ロジックへ置換。`tooling_registry.json` の保存形式も簡素化し、旧形式からの読み込み互換を追加した。

#### ツール環境情報を Rez パッケージ+バリアントへ統一
- **現象**: ツール環境の管理項目に名前や環境変数などが残り、Rez パッケージ＋バリアントだけに統一する運用方針と一致していなかった。
- **原因**: 環境定義と UI が旧来の項目を保持したまま簡素化されておらず、保存形式にも余剰項目が残っていた。
- **対応**: ツール環境定義から name/環境変数/メタデータ等を排除し、Rez パッケージとバリアントのみで識別・保存する形に統一した。UI も同様に Rez パッケージとバリアントの入力に絞った。
...
### ヘッドレス起動の待機後終了を手動終了扱いへ統一
- **現象**: ヘッドレス起動で 10 秒待機後に終了させたい運用に対し、`auto_exit` 判定が混在して起動確認の成功基準がぶれていた。
- **原因**: ヘッドレス起動でも `auto_exit` を理由に設定していたため、手動終了扱いでの成功判定に揃わなかった。
- **対応**: ヘッドレス起動では `auto_exit` 判定を付けず、待機後の終了を手動終了扱い（成功）とするよう修正した。

### ツール環境ノードの構文エラーによる起動失敗
- **現象**: アプリケーション起動時に `tool_environment.py` の構文エラーで即時終了する。
- **原因**: `_update_summary` の `try` ブロック内が空になり、インデントエラーが発生していた。
- **対応**: 不要な `try` を削除して構文エラーを解消した。
