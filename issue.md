...
#### ツール環境ノードからツールを起動できない
- **現象**: ツール環境ノードのプロパティから直接ツールを実行する導線がなく、Rez 設定を確認しても最終的な起動は手動で行う必要があった。
- **原因**: ノードに紐づく Rez 環境を組み立てて実行する UI とサービスメソッドが未実装だった。
- **対応**: インスペクタに「起動」ボタンを追加し、選択中ノードの Rez パッケージ・バリアント・環境変数を組み合わせて rez env コマンド経由で登録済みツールを起動できるようにした。
