...
### ファイルノード削除後のフォルダ残存と保存時同期漏れ
- **現象**: ノードグラフからファイルノードを削除しても、プロジェクト配下の `file_nodes` フォルダが残存する。また、プロジェクト保存時にノード側のファイルが未コピー/未相対化でも処理が完結せず、ノードグラフ上の状態とプロジェクト配下の内容に差異が残ることがあった。
- **原因**: 保存時にノードグラフ上のファイルノード集合とプロジェクト配下の `file_nodes` ディレクトリが照合されず、不要フォルダの掃除が未実装だった。
- **対応**: プロジェクト保存時にファイルノードの同期処理後、ノードグラフに存在しない UUID のフォルダを削除するクリーンアップを追加し、未相対化ノードは引き続き同期処理で確実にプロジェクト配下へコピー・相対化するよう整理した。

### ファイルノード削除処理の長いパス対応不足
- **現象**: Windows 環境で長いパスを持つファイルノードを削除すると、パス長制限により削除に失敗することがあった。
- **原因**: 削除処理が通常のパスをそのまま `unlink` / `rmtree` に渡しており、長いパスの正規化が行われていなかった。
- **対応**: ファイルノード削除時に `\\\\?\\` 形式への正規化を適用し、長いパスでも削除できるよう統合した。

### ファイルノード削除時の不存在エラー
- **現象**: ファイルノード削除の確認後に `[WinError 3] 指定されたパスが見つかりません。` が表示されることがあった。
- **原因**: 削除対象のファイルが既に存在しない場合でも `unlink` / `rmtree` を実行しており、長いパス対応後も `FileNotFoundError` が警告扱いになっていた。
- **対応**: 長いパスの正規化後に `os.path.exists` を確認し、存在しないパスはスキップするよう修正した。`FileNotFoundError` も警告を出さずに無視する。
