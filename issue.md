...
### ジャンクション生成のコピー/リンク分離と後始末強化（2026-04-08）
- **現象**: ファイルノードのローカル展開時にコピーとジャンクション生成が同じ走査内で行われ、途中キャンセルや競合時に生成済みリンクが残留する可能性があった。
- **原因**: `_build_junction_tree` がコピー確認・コピー処理とジャンクション生成を同時に進める設計で、途中中断時の後始末が難しかった。
- **対応**: 入力ノード収集 → ローカルコピー確認/実行 → ジャンクション生成の2段階に分離し、生成中断時には新規作成したリンクのみを削除する後始末を追加した。
