...
### ツール環境の入力プラグ参照が未対応（2026-03-31）
- **現象**: ツール環境編集で環境変数・起動引数・要求ファイルにファイルノードのパスをディレクトリとして参照できず、環境定義がノードとして再利用できなかった。
- **原因**: ツール環境ファイルに入力プラグの定義と参照符号が存在せず、ノード側にも環境ペイロードを保持するプロパティが用意されていなかった。
- **対応**: 入力プラグの定義と `{{input:プラグ名}}` の符号を導入し、環境ファイルへ保存・読み込み・ノードプロパティへの反映を実装した。
