...
### ローカルコピーとジャンクション生成導線の追加（2026-04-05）
- **現象**: ディレクトリを保持するノードにユーザーローカルへコピーする導線が無く、ファイルノードでも入力側ノード構造をローカルで再現できなかった。
- **原因**: インスペクタにローカルコピー／ローカルで開く UI が存在せず、ノード単位のローカルコピーやジャンクション生成ロジックが未実装だった。
- **対応**: ノードインスペクタに「ローカルへ移動」「ローカルで開く」ボタンを追加し、ユーザーローカル配下 `nodes` へノード UUID でコピーする処理と、`junc` へ入力側ノード構造を反映したジャンクション生成フローを追加した。
