...
### ツールノードの多段折り畳み不足と更新フローの硬直化
- **現象**: コンテンツブラウザのツールノードはジャンル直下の単一階層に並ぶため、同一ツールのバージョン差分をまとめて折り畳めず、更新処理もフラット前提で拡張しづらかった。
- **原因**: ノードカタログが「ジャンル → ノード」の2階層固定で、ツール名やバージョン情報を持たずに UI 側でグルーピングできない構造だった。
- **対応**: ツール名とバージョンをカタログデータに追加し、コンテンツブラウザで「ジャンル → ツール → バージョン」の階層表示へ刷新した。フィルタ処理・件数表示・アイテムサイズ更新も多段構造に対応させ、更新フローを柔軟にした。

### ツール折り畳み集計で起動時クラッシュ
- **現象**: コンテンツブラウザ更新時に `TypeError: unhashable type: 'PySide6.QtGui.QStandardItem'` が発生し、ノードエディタ起動時にクラッシュした。
- **原因**: フィルタ処理の集計辞書に `QStandardItem` をキーとして使用していたため、ハッシュ化できず例外となった。
- **対応**: 集計キーを `QStandardItem` ではなく `(ジャンル, ツール名)` のタプルへ変更し、折り畳み件数と表示更新を安定化した。

### ツール再起動後にアイコンが既定に戻る
- **現象**: ツール再起動後にコンテンツブラウザのツールアイコンが既定のアイコンへ戻り、手動で認識しづらくなった。
- **原因**: Rez パッケージから実行ファイルを解決できない場合にアイコンパスが欠落し、既定アイコンへフォールバックしていた。
- **対応**: 取得済みのツールアイコンパスを設定ストアへ保持し、再起動後でもキャッシュを参照してアイコンを復元するようにした。

### ツール階層の最下層表示が縦並びになっていた
- **現象**: ツール階層化後の最下層（バージョン）項目が縦並び表示となり、一覧性が低下していた。
- **原因**: 既定の表示フォーマットが複数行前提のままで、ツール階層の最下層でも改行を含んでいた。
- **対応**: ツール配下の最下層項目では 1 行表示に統一し、アイコンと項目が横並びで視認しやすくなるようにした。

### ツール最下層の高さが2行分確保されていた
- **現象**: 最下層のツール項目を 1 行表示に切り替えても、行の高さが 2 行分確保されて横並びに見えなかった。
- **原因**: コンテンツブラウザの一覧サイズヒントが 2 行前提のままで、ツール項目に対しても同じ高さが適用されていた。
- **対応**: ツール項目に限り 1 行用のサイズヒントを適用し、最下層の表示が横並びで収まるよう調整した。
