...
#### 環境定義ダイアログの複数選択対応
- **現象**: ツール環境定義の一覧が単一選択のみで、複数の環境をまとめて削除したい要望に応えられず、誤操作防止のためのボタン活性制御も不足していた。
- **対応**: 一覧を複数選択可能にし、削除ボタンは選択件数に応じて有効化、編集ボタンは単一選択時のみ有効化する動的制御を追加した。複数削除時の確認ダイアログと削除結果のフィードバックも強化した。

#### ツール登録ダイアログの複数選択削除不足
- **現象**: ツール登録一覧が単一選択のみ対応で、Shift などを使った複数選択や一括削除ができなかった。
- **原因**: 選択モードが `SingleSelection` に固定され、削除処理も単一項目前提になっていた。
- **対応**: 選択モードを複数選択 (`ExtendedSelection`) に変更し、選択件数に応じた確認メッセージと削除処理、ボタン活性制御を追加した。
