...
### Rez 起動手順の失敗箇所特定不足（2026-03-30）
- **現象**: Rez 起動処理で失敗した場合、どのステップでエラーになったかが UI 上で判別しづらかった。
- **原因**: 起動処理をまとめて実行しており、例外にステップ情報が付与されていなかった。
- **対応**: Rez 起動の各ステップにラベルを付与し、例外メッセージに `[step:...]` を付加することで、失敗箇所を明確にした。
