...
### ツール環境編集でファイルノードをディレクトリ参照できない
- **現象**: 起動環境編集ダイアログの要求ファイル・環境変数・起動引数で、ファイルノードをディレクトリとして参照する記法が存在せず、入力の統一が難しかった。
- **原因**: ファイルノード参照の符号定義が用意されておらず、ツール環境編集 UI でも UUID を安全に扱う導線が欠けていた。
- **対応**: `file-node-dir://<UUID>` 形式を含むファイルノード参照の定義と正規化ヘルパーを追加し、要求ファイルにファイルノード参照を追加できる UI と、環境変数/起動引数で参照を正規化する処理を統合した。
