...
#### ツール環境ノードのアイコンが環境定義の .exe を反映しない
- **現象**: コンテンツブラウザのツール環境ノードが、環境定義で指定した .exe のアイコンではなく、ツール登録側の実行ファイルアイコンを表示していた。
- **原因**: ノードカタログ生成時に `ToolEnvironmentDefinition` の環境変数・メタデータ内の .exe を参照せず、ツール登録 (`RegisteredTool`) の `executable_path` を固定で使用していた。
- **対応**: 環境定義の `rez_environment` と `metadata` から `.exe` パスを抽出して優先的に `icon_path` に採用し、環境定義で指定された実行ファイルのアイコンが表示されるようにした。
