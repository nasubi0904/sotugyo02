...
### ツール環境編集でノード入力ディレクトリを反映できない
- **現象**: ツール環境の編集で要求ファイル・環境変数・起動引数にファイルノード由来の入力ディレクトリを指定しても、環境定義ファイルやツール環境ノードへ反映できず、コンテンツブラウザの環境定義にも現れなかった。
- **原因**: ツール環境編集ダイアログが JSON を標準出力へ出すだけで、KDMenvs 配下の環境ファイルやツール環境定義へ保存していなかった。また、環境ファイルを読み取ってノード入力プラグや環境定義へ同期する仕組みが未整備だった。
- **対応**: ノード入力ディレクトリの符号定義 (`{{input_dir:入力名}}`) を追加し、環境ファイル保存・読み込みとツール環境定義への同期を実装した。さらにツール環境ノード生成時に入力プラグを追加し、環境定義をコンテンツブラウザへ反映できるよう統合した。
