...
### Rez 環境変数取得時のエラー情報不足
- **現象**: `rez-env` 実行に失敗した際、LaunchError に実際に使用した `rez-env` の実体パスや `REZ_PACKAGES_PATH` が含まれず、現場での環境差分特定に時間がかかった。
- **原因**: `_collect_execute_vars_via_rez_env()` の `subprocess.run()` 失敗時メッセージが例外内容のみで、実行時の環境情報を添付していなかった。
- **対応**: 例外メッセージ末尾に `rez_env=...` と `REZ_PACKAGES_PATH=...` を追記し、エラーログだけで環境差分を判断できるようにした。
