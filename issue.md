...
#### ツール環境ノードのバージョン識別子不足
- **現象**: 同一ツールの複数バージョンを登録しても、ノードタイプが環境 ID だけで生成されるため、コンテンツブラウザ上でバージョン違いが衝突しやすかった。
- **原因**: `build_tool_catalog` が `environment_id` のみを `node_type` に採用し、ツール環境定義側でもバージョンを含めた識別子を提供していなかった。
- **対応**: ツール環境定義にバージョンを含むノード識別子生成ヘルパーを追加し、カタログ生成時は `tool-environment:<env_id>@<version>` 形式で登録するよう更新。ノード生成側もバージョン付きキーで環境定義を参照するようにした。
