...
### コンテンツブラウザの既定フォルダ保護と環境定義の再編
- **現象**: ワークフロー/環境定義の既定フォルダ内で削除や並び替えが可能で、既定構成が意図せず編集されてしまう。さらに環境定義のツール/環境が単一フォルダに混在し、操作導線が曖昧だった。
- **原因**: 既定フォルダ配下の編集制御が削除アクションの保護に留まっており、ドラッグ移動や並び替えの制限が不足していた。また環境定義フォルダ内にサブフォルダを持たせる構造が未整備だった。
- **対応**: 既定フォルダ配下の項目はコピーのみ許可し、削除や並び替えを禁止するガードを追加。環境定義配下に「ツール」「環境」フォルダを新設し、ツール環境ノードはツール配下へ集約するよう配置ルールを更新した。
