...
### ディレクトリノードのローカル移動導線不足（2026-04-05）
- **現象**: ディレクトリパスを保持するファイルノードから、ユーザーローカルディレクトリ配下の `nodes/<uuid>` へ一括コピーする導線が存在せず、手動で同期する必要があった。
- **原因**: インスペクタにローカルコピー操作が用意されておらず、ノード UUID を使った保存先ディレクトリ生成も未整備だった。
- **対応**: インスペクタに「ローカルへ移動」ボタンを追加し、ディレクトリノード選択時のみ表示する。押下時にユーザーローカルディレクトリ配下 `nodes/<uuid>` を作成し、ディレクトリ全体をコピーする処理を追加した。
