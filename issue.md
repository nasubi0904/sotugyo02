...
### ツールノードのプロパティ画面に起動ボタンがない
- **現象**: ツールノード選択時にインスペクタのプロパティ画面からツール起動ができず、Rez 環境の起動導線が不足していた。
- **原因**: インスペクタにツール起動 UI と起動処理が実装されておらず、rez_info を利用した起動ヘルパーとの連携が存在しなかった。
- **対応**: インスペクタのプロパティタブにツール起動ボタンを追加し、選択中のツールノードから rez_info を取得して `scripts/rez_launch.py` の起動処理を呼び出すようにした。

### Rez 起動ログのパス生成で Windows エラーが発生する
- **現象**: ツール起動時に `WinError 123` が発生し、ログ保存ディレクトリの作成に失敗する。
- **原因**: `rez_launch._make_log_path` が `tool_args[0]` のフルパスをそのままログファイル名へ埋め込み、Windows の禁止文字（`:` や `\\`）を含むファイル名になっていた。
- **対応**: `tool_args[0]` を `Path(...).name` でベース名化し、ファイル名として不適切な文字を `_sanitize_log_token` で除去するよう修正した。

### Rez 起動時に package.py を実行して失敗する
- **現象**: ツール起動時の Rez ログに `package.py` 実行失敗が記録され、実行コマンドが `package.py` を指していた。
- **原因**: Rez パッケージに紐づく実行ファイルを解決できない場合、`RegisteredTool.executable_path` が `package.py` へフォールバックしており、そのまま起動コマンドに使用していた。
- **対応**: 起動時に `package.py` を直接使わず、rez パッケージ情報から実行ファイルを再解決するように修正し、解決できない場合は起動を中断するようにした。
