...
### 起動環境の色空間走査を DCC 別ルールへ刷新
- **現象**: 起動環境の追加・編集ウィンドウで色空間候補が Rez パッケージディレクトリの単純走査に依存していたため、実際の DCC 同封構成を反映できなかった。
- **原因**: UI 層がパッケージディレクトリを直接走査し、インストール実体の場所や DCC ごとの OCIO 同封パスを考慮していなかった。
- **対応**: ツール登録情報・Rez パッケージから実行ファイルを特定し、DCC 別の既定探索パスと環境変数に基づいて候補を抽出するサービスへ移行した。候補の表示は「環境変数名 + 同封パス」を示す形式へ更新した。
