...
- **現象**: 起動環境編集ダイアログに要求プラグインの一覧と追加導線がなく、環境変数より前に必要なプラグイン情報を登録できなかった。
- **原因**: UI 再構成時にプラグインの登録エリアが未設計のまま残り、ファイルパスの保持形式（absolute/known）も定義されていなかった。
- **対応**: 要求プラグインのリスト表示と追加ボタン、Known Folder を使った相対化チェックを追加し、拡張子統一チェックと absolute/known 属性を付けたパス保持ロジックを実装した。
