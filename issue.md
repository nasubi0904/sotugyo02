...
### ノードローカルコピーとジャンクション生成の導線不足（2026-04-05）
- **現象**: ディレクトリを保持するノードやファイルノードに対し、ユーザーローカル `nodes` へのコピーや `junc` 生成の導線がなく、ノード構造をローカルに再現できなかった。
- **原因**: インスペクタにローカルコピー/ローカルで開くボタンが存在せず、ノード入力構造を辿ってジャンクションを生成する処理も未実装だった。
- **対応**: ノードインスペクタへローカルコピー/ローカルで開くの操作ボタンを追加し、ユーザー設定のローカルディレクトリ配下に `nodes`/`junc` を生成する機能を実装した。ファイルノードの入力側グラフから構造を抽出し、`junc` にノード名ディレクトリと `data` ジャンクションを作成するよう整理した。
