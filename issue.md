...
#### ツール起動ダイアログの環境変数表示不足 (2025-12-19)
- **現象**: ツール起動時に使用する環境変数が確認できず、環境依存のトラブル切り分けが難しかった。
- **原因**: ノードに保存されている `rez_environment` を起動成功ダイアログで表示していなかった。
- **対応**: 起動成功ダイアログに `rez_environment` の内容を追記し、コマンドとあわせて確認できるようにした。

#### ツール環境と Rez パッケージの重複表示 (2025-12-19)
- **現象**: 同じパッケージ名のツール環境と Rez パッケージが並び、コンテンツブラウザに重複して表示された。
- **原因**: ツール環境カタログと Rez パッケージカタログを別々に追加していた。
- **対応**: ツール環境に紐づくパッケージは Rez パッケージ一覧から除外し、1件に統合して表示するようにした。
