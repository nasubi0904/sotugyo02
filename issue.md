...
### ツール起動環境構成ダイアログのUI再実装
- **現象**: 起動環境の構成画面が「再構成中」の文言のみで、Rez commands 編集と起動引数コネクタ定義を同一画面で編集できなかった。
- **原因**: 既存 UI が暫定プレースホルダーのまま残っており、ヘッダー入力欄や2分割編集領域の構築が未実装だった。
- **対応**: 環境ノード情報ヘッダー、Rez commands 編集領域、起動引数コネクタ定義領域を1ウィンドウ内に分割配置し、プレビューと検証ボタンを含む UI を実装した。
