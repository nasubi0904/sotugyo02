...
### Rez 起動時のバージョン不一致による EXECUTE_ 取得失敗
- **現象**: ツール環境ノードで Rez 起動を実行すると「EXECUTE_ 変数の取得に失敗しました（rez-env returncode=1）」が発生し、起動できない。
- **原因**: ツールノードの `rez_info.version` で指定されたバージョンがローカルの Rez パッケージに存在しない場合でも起動対象がそのまま `rez-env <name>-<version>` に渡され、パッケージ解決が失敗していた。
- **対応**: ローカルに存在する Rez パッケージのバージョン情報を参照し、指定バージョンが見つからない場合は利用可能なバージョン（またはバージョン指定なし）へフォールバックするよう起動処理を調整した。起動時にはダイアログでバージョン調整の旨を通知する。
