...
### 起動環境構成ダイアログのUI試作
- **現象**: 起動環境の構成画面が再構成中のままで、登録済み環境の一覧確認や編集導線が存在しなかった。
- **原因**: 一時的なプレースホルダー表示のみで、環境リスト・編集モーダル・環境変数/起動引数の入力UIが未実装だった。
- **対応**: KDMenvs 管理を想定した環境一覧、選択時に有効化される編集ボタン、空の追加モーダルを実装した。ツールパッケージのプルダウンと色空間候補の検出/選択UI、環境変数・起動引数の入力テーブルを追加し、環境作成ボタンでデモ出力を行う構成へ更新した。
