...
### ファイルノード保存時の更新日時差分確認と手動検証導線不足
- **現象**: プロジェクト保存時にファイルノードの参照ファイルをプロジェクト配下へコピーする際、更新日時の新旧比較がなく常に上書きされる可能性があった。また、ファイルノードのパスをユーザーが手動でプロジェクト相対へ変換・検証する導線が存在しなかった。
- **原因**: ファイルノード同期処理が更新日時比較の分岐を持たず、インスペクタに相対パス変換/検証ボタンが未実装だった。
- **対応**: 保存前に参照元とプロジェクト側の更新日時を比較し、同一ならスキップ、参照元が新しければ上書き、プロジェクト側が新しい場合は警告と選択肢を表示するよう整理した。さらにインスペクタに「パスを検証/変換」ボタンを追加し、プロジェクト配下なら相対パスへ変換できるようにした。
