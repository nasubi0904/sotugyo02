...
### ノード名に応じた横幅伸長と色の識別不足（2026-04-05）
- **現象**: ノード名が長い場合にノード幅が自動伸長し、ノードごとの横幅が揃わない。さらにデモノード（タスク/レビュー）の色が既定色のままで、種類の区別がつきにくい。
- **原因**: ノード幅を固定する共通定数が無く、NodeGraphQt の既定サイズがノード名長に応じて変化していた。デモノードには色設定が行われていなかった。
- **対応**: 共通のノード幅定数を追加し、メモノード以外の各ノードで幅を固定。タスクノードとレビューノードに固有の色を設定し、ツール環境ノードは色調を変更して識別性を向上させた。
