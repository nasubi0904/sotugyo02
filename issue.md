...
### ファイルノード保存時の更新日時判定と手動変換導線
- **現象**: プロジェクト保存時にファイルノードが参照する外部ファイルとプロジェクト側コピーの更新日時を比較しておらず、既存ファイルの方が新しい場合でも無条件に上書きされる可能性があった。ファイルノードのパスをユーザーが手動でプロジェクト相対へ検証・変換する導線も不足していた。
- **原因**: 保存同期が「存在するならコピーする」前提になっており、タイムスタンプ比較やユーザー選択の分岐を実装していなかった。また、インスペクタにファイルノード用のパス検証ボタンが無かった。
- **対応**: ファイルノード同期時に NTFS の更新日時（`st_mtime_ns`）を取得し、同一ならコピーを省略、プロジェクト側が古ければ上書き、プロジェクト側が新しければ確認ダイアログで選択させるよう整理した。さらにインスペクタへ「パスを検証/変換」ボタンを追加し、プロジェクト相対パスへの手動変換導線を補強した。
