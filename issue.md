...
### KDMenvs 環境定義の反映漏れと編集ダイアログの保存不足
- **現象**: KDMenvs 配下の環境定義ファイルを追加してもツール環境一覧に反映されず、編集ダイアログで作成した内容もファイル保存されないため起動後に消える。
- **原因**: ツール環境同期が Rez パッケージ由来の定義のみを生成しており、KDMenvs の json/yaml/toml を読み込む処理が存在しなかった。さらに編集ダイアログは環境内容を標準出力へ出すだけで、保存後の一覧更新フローが不足していた。
- **対応**: `get_tool_environment_dir()` 配下の環境定義ファイルをロードして `ToolEnvironmentDefinition` に変換し、`ToolConfigRepository.save_all()` の同期結果へ統合した。編集ダイアログで環境定義をファイル保存し、保存後に一覧更新とノードエディタ再同期が行われるよう調整した。
