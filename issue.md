...
### ディレクトリコピーで再解析ポイントが再帰対象になる（2026-04-08）
- **現象**: ファイルノード保存時のディレクトリ同期で、Windows のジャンクション/シンボリックリンクを含む場合に予期せぬ再帰やコピー対象の肥大化が起きる可能性があった。
- **原因**: コピー処理が `os.path.islink()` のみを想定しており、NTFS の再解析ポイント（ジャンクション等）を検出できず `os.walk` の `dirs` から除外できていなかった。
- **対応**: `os.lstat` と `stat.FILE_ATTRIBUTE_REPARSE_POINT` を併用して再解析ポイントを検出し、`os.walk` の再帰対象から除外した。除外したパスは欠落ファイルとは別に警告で通知するよう整理した。
