...
### Rez パッケージ保存先の Known Folder 優先化
- **現象**: Windows 環境で Rez パッケージ保存先が環境変数依存のまま決定され、Known Folder を優先できずに既定の保存先とずれる可能性があった。
- **原因**: `get_rez_package_dir()` が `LOCALAPPDATA` / `APPDATA` のみを参照し、`SHGetKnownFolderPath` による Known Folder 解決を行っていなかった。
- **対応**: `SHGetKnownFolderPath` で `FOLDERID_LocalAppData` を解決できる場合はそのパスを最優先で採用し、取得失敗時のみ `LOCALAPPDATA` → `APPDATA` → `Path.home()` の順でフォールバックするよう整理した。
