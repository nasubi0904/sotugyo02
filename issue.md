...
### プロジェクト保存時の Rez パッケージ出力不足
- **現象**: プロジェクト保存時にツールノードで使用している Rez パッケージ一覧がプロジェクト配下へ保存されず、外部から依存関係を参照できなかった。
- **原因**: ノードグラフ保存処理では Rez パッケージのコピーは行っていたが、プロジェクト名を用いた Rez パッケージマニフェスト（`package.py`）の生成が実装されていなかった。
- **対応**: ツールノードの Rez パッケージ名とバージョンを収集し、`config/rez_packages/<project>/1.0/package.py` を生成して `requires` に列挙する処理を追加した。

### プロジェクト保存時の Rez パッケージ保存範囲の見直し
- **現象**: プロジェクト保存時に Rez パッケージ本体も同期され、プロジェクトサイズが肥大化していた。
- **原因**: 保存処理が Rez パッケージのコピーとマニフェスト出力の両方を実行していた。
- **対応**: プロジェクト保存ではマニフェスト (`package.py`) のみを出力し、パッケージ本体の同期を行わないよう調整した。

### プロジェクト展開時の Rez 依存チェック不足
- **現象**: プロジェクトの Rez マニフェストに記載された依存がローカル環境に存在するか確認されず、起動後に不足が発覚していた。
...
### プロジェクト展開時の Rez マニフェスト依存チェックで KDMrez が参照されない
- **現象**: プロジェクト展開時のマニフェスト依存チェックで KDMrez 配下のパッケージが見つからず、ローカルに存在するはずの依存が不足扱いになるケースがあった。
- **原因**: Rez Python API の照会時に `REZ_PACKAGES_PATH` を補完しておらず、KDMrez が検索パスに含まれていない環境で存在確認が失敗していた。
- **対応**: 依存チェック実行時に `REZ_PACKAGES_PATH` へ KDMrez を一時的に追加し、Rez の照会対象に含めるよう修正した。

### ツールノードの Rez 参照が表示名依存になっていた
- **現象**: プロジェクト保存や依存チェックで Rez パッケージ名を取得する際、ノード表示名（`Rez:` プレフィックス）を解析しており、表示名変更で参照がずれる可能性があった。
- **原因**: ツールノードに Rez の name/version を保持する専用メタデータが存在せず、文字列解析に依存していた。
- **対応**: ツールノードのカスタムプロパティに `rez_name` と `rez_version` を保存し、Rez 参照はカスタムプロパティ経由で取得するよう統一した。

### Rez カスタムプロパティ形式の再整理
- **現象**: `rez_name` / `rez_version` が個別キーで保存されており、関連情報が分散していた。
- **原因**: Rez 情報の構造化（name/version のまとまり）を想定した設計が不足していた。
- **対応**: `rez_info` に `{name, version}` を保持する形式へ移行し、参照は `rez_info` を優先するよう整理した。
