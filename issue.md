...
### ツールノードの起動ボタン未実装
- **現象**: ツールノードのプロパティから起動操作ができず、rez_info を元にローカル Rez パッケージを起動する導線が存在しなかった。
- **原因**: ノードインスペクタに起動ボタンを設けておらず、rez_info と Rez パッケージの解決・起動を結び付ける処理が UI 側に実装されていなかった。
- **対応**: ノードプロパティに起動ボタンを追加し、rez_info を参照してローカル環境（KDMrez）からパッケージを特定し、rez_launch.py 経由でツールを起動する処理を実装した。

### ツール起動コマンドに拡張子が含まれる
- **現象**: `rez_launch` 呼び出し時の起動コマンドが `.exe` を含むため、環境によっては起動失敗や警告が発生した。
- **原因**: ツール起動時に `tool_path.name` / `resolved.name` をそのまま渡しており、コマンド名に `.exe` 拡張子が付与されたままだった。
- **対応**: 起動コマンドは `.exe` を除外した名前 (`Path.stem`) を使用するよう修正した。
