...
### ノード幅のばらつきと色分け不足（2026-04-05）
- **現象**: ノード名が長い場合にノード幅が伸び、グリッド幅で揃わない。また、ノード種別ごとの色が揃っておらず識別しづらかった。
- **原因**: NodeGraphQt の既定レイアウトがノード名の長さで幅を広げる仕様のままで、ノード生成・読み込み時に統一幅へ補正する処理が無かった。色設定も一部ノードのみに留まり、デモノードなどは既定色のままだった。
- **対応**: 背景縞のグリッド幅に合わせてメモノード以外のノード幅を強制的に統一する処理を追加し、プロジェクト読み込み時も同様に補正するよう整理した。併せてノード種別ごとの色を設定して視認性を向上させた。

### ノード名の省略表示不足（2026-04-05）
- **現象**: 長いノード名が表示領域を超えると、ノード幅が伸びるか、表示がはみ出して視認性が落ちる。
- **原因**: ノード名の表示を省略（`...`）する処理がなく、ラベルの幅がそのままノードサイズ計算に影響していた。
- **対応**: ノード名を表示領域に合わせて省略表示し、ノードの実名は維持したまま描画テキストを調整するようにした。名前変更後やプロジェクト読み込み時にも省略表示が更新されるよう整理した。

### 省略ラベルの位置ずれ（2026-04-05）
- **現象**: 省略表示を適用したノード名が中央からずれて描画され、見た目が不自然になる。
- **原因**: ノード名の描画テキストを直接更新した際に、NodeGraphQt のラベル再配置処理が呼ばれず位置が古いまま残っていた。
- **対応**: 省略表示を更新した直後にラベル再配置を実行し、ノード幅に対して中央揃えとなるよう補正した。
