...
### ツール起動環境の構成 UI の再設計
- **現象**: ツール起動環境の構成画面が簡易表示のままで、Rez commands 編集と起動引数コネクタ定義を分離して操作できなかった。
- **原因**: UI 再構成の要件が整理されていなかったため、構成要素（2 ペイン / 2 タブ、コネクタ詳細、プレビューなど）を満たす画面が未実装だった。
- **対応**: 2 タブ構成の UI を作成し、Rez commands 編集エリアと起動引数コネクタ定義エリアを分離した。引数定義の一覧・詳細・プレビュー・コメント JSON の各 UI を用意し、実機能なしで表示要件を満たす構成へ更新した。

### ツール起動環境の構成ダイアログのモーダル化と確認表示
- **現象**: ダイアログがモーダルではなく、環境作成ボタンの押下時に入力内容を確認できなかった。
- **原因**: モーダル設定と、入力内容をまとめて表示する UI が不足していた。
- **対応**: ダイアログをモーダル化し、「環境を作成」ボタン押下時に入力内容をメッセージで表示する確認 UI を追加した。
