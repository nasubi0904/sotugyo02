...
def test_load_all_returns_empty_when_storage_missing(tmp_path: Path) -> None:
    repository = ProjectRezPackageRepository(tmp_path)
...
def test_load_all_recovers_from_corrupted_payload(tmp_path: Path) -> None:
    repository = ProjectRezPackageRepository(tmp_path)
    storage_path = repository.resolve_storage_path()
    storage_path.parent.mkdir(parents=True, exist_ok=True)
    storage_path.write_text("{this is invalid json}", encoding="utf-8")
...
def test_load_all_ignores_non_mapping_entries(tmp_path: Path) -> None:
    repository = ProjectRezPackageRepository(tmp_path)
    storage_path = repository.resolve_storage_path()
    storage_path.parent.mkdir(parents=True, exist_ok=True)
    payload = [{"environment_id": "env-001", "tool_id": "toolA"}, "invalid", 42]
    storage_path.write_text(json.dumps(payload), encoding="utf-8")

    loaded = repository.load_all()

    assert len(loaded) == 1
    assert loaded[0].environment_id == "env-001"


def test_save_all_creates_storage_directory(tmp_path: Path) -> None:
    repository = ProjectRezPackageRepository(tmp_path)
    entry = _build_entry("env-001")

    repository.save_all([entry])

    assert repository.resolve_storage_path().exists()
...
def test_update_from_node_returns_false_when_unchanged() -> None:
    entry = _build_entry("env-001")
    before_timestamp = entry.updated_at

    changed = entry.update_from_node(
        tool_id=entry.tool_id,
        packages=entry.rez_packages,
        variants=entry.rez_variants,
